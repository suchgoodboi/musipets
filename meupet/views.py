...
class RegisterSongView(LoginRequiredMixin, CreateView):
    template_name = 'meupet/addSong.html'
    model = models.Song
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user == current_song.owner:
            ...
            return HttpResponseRedirect(
                reverse('meupet:detail', kwargs={'pk_or_slug': current_song.slug})
            )
...
def change_status(request, slug):
    ...
    song.change_status()
    return HttpResponseRedirect(reverse('meupet:detail', kwargs={'pk_or_slug': song.slug}))
...
class SearchView(View):
    ...
    def post(self, request):
        ...
        return render(request, 'meupet/search.html', {'form': form, 'songs': songs})
...
def poster(request, slug):
    song = get_object_or_404(models.Song, slug=slug)
    return render(request, 'meupet/poster.html', {'song': song})
